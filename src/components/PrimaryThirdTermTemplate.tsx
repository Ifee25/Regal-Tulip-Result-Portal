...
export default function PrimaryThirdTermTemplate({ student_name, session, term, class_name, priorTermResults, onSubmit, onCancel, initialResult, readOnly = false, draftKey, onEdit, submitLabel = "Submit Result" }: Props) {
  ...
  const score = (index: number, key: keyof PrimarySubjectResult, max = 100, locked = false) => {
    const isClassSummary = key === "class_highest_score" || key === "class_lowest_score";
    return <input type="text" inputMode="decimal" readOnly={locked} value={subjects[index].not_offered && !isClassSummary ? "N/A" : scoreDrafts[`${index}:${String(key)}`] ?? subjects[index][key] as number | undefined ?? ""} onChange={(e) => updateSubject(index, key, e.target.value)} onBlur={() => finishScoreEdit(index, key)} className={`h-full w-full bg-transparent px-0.5 text-center outline-none ${locked ? "cursor-default" : ""}`} aria-label={`${subjects[index].subject} ${String(key)}`} data-max={max} />;
  };
