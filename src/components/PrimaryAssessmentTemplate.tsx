...
export default function PrimaryAssessmentTemplate({ student_name, session, term, class_name, onSubmit, onCancel, initialResult, readOnly = false, draftKey, onEdit, submitLabel = "Submit Result" }: Props) {
  ...
  const scoreInput = (row: number, key: keyof PrimarySubjectResult, max?: number, locked = false) => (
    <input type="text" inputMode="decimal" min={0} max={max} readOnly={locked} value={subjects[row].not_offered && (key === "cat" || key === "exam") ? "N/A" : scoreDrafts[`${row}:${String(key)}`] ?? subjects[row][key] as number | undefined ?? ""} onChange={(e) => updateSubject(row, key, e.target.value)} onBlur={() => finishScoreEdit(row, key)} className={`h-full w-full bg-transparent px-1 text-center outline-none ${locked ? "cursor-default" : ""}`} />
  );
